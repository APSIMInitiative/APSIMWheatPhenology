...
readPara <- function( name, ... )
{
    ...
    if ( !exists( "rapsim.glb.para" ) | !is.null( sim ) )
    {
        if ( !file.exists( sim ) )
        {
            stop( paste( sim, " not existed.", sep = "" ) )
        }
        assign( "rapsim.glb.para", xmlInternalTreeParse( sim ), envir = .GlobalEnv )
    }
    ...
    {
        t.name <- name[i]
        temp <- xpathApply( rapsim.glb.para, paste( "//", t.name, sep = "" ) )
        ...
        c.res <- xmlValue( temp[[1]] )
        # for genotype parameters 
        if ( length( temp ) > 1 )
        {
            parent <-  xmlParent( temp[[1]])
            if ( xmlName( parent ) == "base_cultivar" )
            {
                # find cultivar
                if (is.null(cultivar_n))
                {
                    cultivar <- xpathApply( rapsim.glb.para, paste( "//", "cultivar", sep = "" ) )
                    if ( length( cultivar ) == 0 )
                    {
                        stop( "Cultivar node can not be find" )
                    }
                    cultivar <- xmlValue( cultivar[[1]] )
                } else
                {
...
getPara4Genotype <- function( name, cultivar )
{
    ...
    if ( length( temp ) > 0 )
    {
        return( xmlValue( temp[[1]] ) )
    } else
    {
        cultivar <- cultivar[[1]]["derived_from"]
        cultivar <- xpathApply( rapsim.glb.para, paste( "//", xmlValue( cultivar[[1]] ) , sep = "" ) )
